...
*/
export  var CryptoJS = CryptoJS || function(h, s) {
	var f = {},
		t = f.lib = {},
		g = function() {},
		j = t.Base = {
	...
	var a = [],
		f = f.SHA256 = g.extend({
			...
			_doProcessBlock: function(c, d) {
				for (var b = this._hash.words, e = b[0], f = b[1], m = b[2], h = b[3], p = b[4], j = b[5], k = b[6], l = b[7], n =
						0; 64 > n; n++) {
					if (16 > n) a[n] =
						c[d + n] | 0;
					else {
						var r = a[n - 15],
							g = a[n - 2];
						a[n] = ((r << 25 | r >>> 7) ^ (r << 14 | r >>> 18) ^ r >>> 3) + a[n - 7] + ((g << 15 | g >>> 17) ^ (g << 13 |
							g >>> 19) ^ g >>> 10) + a[n - 16]
					}
